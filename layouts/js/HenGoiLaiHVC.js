...
function fetchDataHenGoiLaiHVC() {
    ...
    fetch(localStorage.getItem("http_endpoint") + 'obccos/HenGoiLaiHVC?progId=66e951ca7da1b78c6857a7c9&states=4&page=0&size=10000', {
        method: "GET",
        headers: {
...
function reloadTable(data) {
    ...
    $('#scroll-horizontal-datatable').DataTable({
        ...
        createdRow: function (row, data, dataIndex) {
            ...
            const button1 = $('<button style="margin-right: 10px" onclick="showPopupHVC(this,\'obccos/GetListUser\')">Click Me</button>').text('Chuyển OB');

            const button2 = $(`<button onclick="showPopup2HVC(this, '${sdt}')">Click Me</button>`).text('Gia hạn');
...
function showPopupHVC(button, url) {
    // Gọi API khi bấm nút
    ...
    document.getElementById("popupHVC").style.display = "block";
}

function showPopup2HVC(button, sdt) {
    ...
    document.getElementById("popupGHHVC").style.display = "block";
    
}

function submitFormGHHVC(button) {
    document.getElementById('btnXacNhanGHHVC').disabled = true;
    ...
    if (user_ccos == null || user_ccos == "") {
        document.getElementById('btnXacNhanGHHVC').disabled = false;
        document.getElementById('loading-screen').style.display = "none";
...
function closePopupHVC() {
    document.getElementById("popupHVC").style.display = "none";
}

function closePopupGHHVC() {
    document.getElementById("popupGHHVC").style.display = "none";
}

function submitFormHVC() {
    document.getElementById('loading-screen').style.display = 'block';
    // Lấy giá trị của hidden input
    ...
    if (selectedValue == null || selectedValue == "") {
        ...
        fetch(localStorage.getItem("http_endpoint") + 'obccos/ChuyenOB?progId=66e951ca7da1b78c6857a7c9&id=' + id + '&ktv=' + selectedValue, {
            method: "PUT",
            headers: {
                Authorization: `Bearer ${localStorage.getItem("access_token")}`,
                "Content-Type": "application/json",
...
// Gọi API để đổ dữ liệu khi DOM đã sẵn sàng
fetchDataHenGoiLaiHVC();